...
public class PostCommentInGalleryFormatTest extends BaseTests{
	...
	public void galleryFormatPostComment() throws InterruptedException {
		...
		js.executeScript("document.getElementsByClassName('section_title comments_form_title')[0].scrollIntoView()");
		Thread.sleep(1000);
		js.executeScript("document.getElementById('comment').textContent='test message 19'");	
		...
		Thread.sleep(2000);		
		String comment = js.executeScript("return document.getElementsByClassName('comment_text')[0].childNodes[0].innerHTML").toString();
